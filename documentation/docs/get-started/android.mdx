...
- Current AppName is `rnExample` and application id is `com.rnexample`
- Also you can see `brand1` and `brand2` in more places.it is a multiple app
  system.

## Configuration

- Go to terminal and run `npx react-native-rename NewAppName` to change your
  app.

**Note:** new app name folder is created , but file is existing old appName.
just copy and and remove existing folder. example `java/com/appName` is need
manual setup. and confirm application id is changed or not in all android native
folder.
